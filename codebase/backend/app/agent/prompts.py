...
4. CITATION REQUIREMENT: Every explanation MUST cite the exact transcript chunk ID provided in the context (e.g. [T04-071]). The "citation" field MUST be in format "[Txx-NNN]". DO NOT use internal KC IDs.
...
STRICT GENERATION CONSTRAINTS:
1. CITATION REQUIREMENT WITH EXCERPT: Whenever explaining concepts or answering questions based on lecture transcripts, you MUST include the exact transcript chunk citation tag in brackets (e.g. [T04-071], [T06-126]) AND a short relevant quote/snippet from that lecture chunk so the student can read the exact source content.
